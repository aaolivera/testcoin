...
namespace Servicios.Impl
{
    public class YobitProvider : IProvider
    {
        ...
        private void CargarMonedas(IEnumerable<string> response, IMercadoCargar mercado, List<string> exclude, List<string> include)
        {
            try
            {
                ...
                foreach (var relacion in pares.pairs)
                {
                    ...
                    {
                        mercado.CargarRelacionEntreMonedas(monedas[0], monedas[1]);
                    }
                }
            }
            catch
        ...
        {
            using (var repositorio = new RepositorioEF(new DDbContext()))
            {
                var nonce = repositorio.Obtener<Nonce>(1) ?? new Nonce();
                nonce.Value++;
                if (nonce.Id == 0) repositorio.Agregar(nonce);
                repositorio.GuardarCambios();
                return nonce.Value.ToString();
            } 
        }
        #endregion
    }
